...
//调用依赖
require(["jquery","template","domReady","formData","cookie","bootstrap"], function($,template,ready,formData) {
    var $openID = $("#openID"), 
        $url = $("#url"),
        $token = $("#token"),
        $textInput = $("#content"),
    ...
    $sendBtn.click(function(e){
        ...
        ajaxData.data = makeSendData(curType);
    ...
    function makeSendData(type){
        ...
        data.FromUserName = "weixin-debug";
        ...
        switch( type ){
            ...
            case "pic":
                ...
                for( var i=0;i<data.Count;i++ ){
                    data.SendPicsInfo.PicList[i] = {};
                    if( list[i] ){
                        data.SendPicsInfo.PicList[i].PicMd5Sum = list[i];
                    }else{
                        data.SendPicsInfo.PicList[i].PicMd5Sum = formData.infoDefault.PicMd5Sum;
                    }
                }
                delete data.Count;
            ...
            case "location_select":
                ...
                for( var i in data.SendLocationInfo ){
                    if( data.SendLocationInfo.hasOwnProperty(i) && data.hasOwnProperty(i) ){
                        delete data[i];
                    }
                }
                break;
